    ...
    for clause_type, words in CLAUSE_KEYWORDS.items():
        for word in words:
          ...
          if pos >= 0:
              ...
              risks.append({"id": len(risks) + 1, "clause_type": clause_type, "risk_level": risk_level, "description": f"命中关键词 {word}", "suggestion": "建议补充责任边界、期限和举证要求。", "start_pos": pos, "end_pos": pos + len(word)})
    return risks or [{"id": 1, "clause_type": ClauseType.OTHER, "risk_level": RiskLevel.NONE, "description": "未发现明显风险", "suggestion": "保持人工复核。", "start_pos": 0, "end_pos": 0}]
